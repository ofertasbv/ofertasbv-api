...
import com.br.oferta.api.service.EstadoService;
...
//    @PreAuthorize("hasAuthority('ROLE_PESQUISAR_CATEGORIA') and #oauth2.hasScope('read')")
    public List<Estado> findAll() {
        return estadoService.findBySort();
    }
...
//    @PreAuthorize("hasAuthority('ROLE_CADASTRAR_CATEGORIA') and #oauth2.hasScope('write')")
    public ResponseEntity<Estado> create(@Validated @RequestBody Estado estado, HttpServletResponse response) {
        Estado salva = estadoService.create(estado);
...
//    @PreAuthorize("hasAuthority('ROLE_PESQUISAR_CATEGORIA')")
    public ResponseEntity<Estado> update(@PathVariable Long id, @Validated @RequestBody Estado estado) {
        try {
            Estado salva = estadoService.findById(id).get();
            if (salva == null) {
                throw new EmptyResultDataAccessException(1);
            }
            BeanUtils.copyProperties(estado, salva, "id");
            estadoService.create(salva);
